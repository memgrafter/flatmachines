...
from flatagents.providers.openai_codex_login import (
    CodexLoginError,
    OAuthCredentials,
...
from _codex_test_helpers import token_for_account
...
async def test_exchange_authorization_code_success(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "flatagents.providers.openai_codex_login.httpx.AsyncClient",
...
async def test_login_openai_codex_saves_auth_file_without_email_prompt(
    ...
    monkeypatch.setattr(
        "flatagents.providers.openai_codex_login.exchange_authorization_code",
        fake_exchange_authorization_code,
    )

    monkeypatch.setattr("flatagents.providers.openai_codex_login.webbrowser.open", lambda *args, **kwargs: True)
