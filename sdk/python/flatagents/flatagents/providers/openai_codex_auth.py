...
def resolve_auth_file(
    *,
    ...
    path = (
        ...
        or (model_config.get("codex_auth_file") if isinstance(model_config, dict) else None)
...
class CodexAuthStore:
    ...
    def load_provider(self, provider: str = DEFAULT_PROVIDER) -> Dict[str, Any]:
        ...
        if not isinstance(cred, dict):
            raise CodexAuthError(
                f"No credentials for provider '{provider}' in {self.auth_file}. Run codex login first."
            )
        return cred
...
async def refresh_openai_codex_token(
    ...
    *,
    ...
    if response.status_code >= 400:
        raise CodexAuthError(f"Token refresh failed ({response.status_code}). Please run codex login again.")
