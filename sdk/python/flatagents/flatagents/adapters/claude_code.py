"""Claude Code CLI runtime for FlatAgents.
...
from .compat import AgentResult, StreamEventCallback
...
class ClaudeCodeExecutor:
    ...
    async def execute(
        ...
        call_system_prompt = input_data.get("_system_prompt")
        call_append_system_prompt = input_data.get("_append_system_prompt")
        ...
        while True:
            result = await self._invoke_once(
                ...
                append_system_prompt=call_append_system_prompt,
            )
            attempt += 1
    ...
    def _build_args(
        ...
        # Prompt control (mutually exclusive). Per-call values override profile config.
        effective_system_prompt = system_prompt if system_prompt is not None else cfg.get("system_prompt")
        effective_append_system_prompt = (
            append_system_prompt if append_system_prompt is not None else cfg.get("append_system_prompt")
        )
        if effective_system_prompt:
            args += ["--system-prompt", effective_system_prompt]
        elif effective_append_system_prompt:
            args += ["--append-system-prompt", effective_append_system_prompt]
