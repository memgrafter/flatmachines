...
class CodexHTTPError(CodexClientError):
    def __init__(self, status_code: int, body: str, message: Optional[str] = None):
...
class CodexClient:
    def __init__(self, model_config: Dict[str, Any], transport: Optional[httpx.AsyncBaseTransport] = None):
        auth_cfg = model_config.get("auth") if isinstance(model_config.get("auth"), dict) else {}
        self.config = CodexClientConfig(
            base_url=str(model_config.get("base_url") or DEFAULT_BASE_URL),
            originator=str(model_config.get("codex_originator") or "pi"),
            timeout_seconds=float(model_config.get("codex_timeout_seconds") or 120),
            max_retries=int(model_config.get("codex_max_retries") or 3),
            refresh_enabled=bool(model_config.get("codex_refresh", True)),
            provider=str(auth_cfg.get("provider") or model_config.get("provider") or "openai-codex"),
            auth_file=model_config.get("codex_auth_file") or auth_cfg.get("auth_file"),
            token_url=str(model_config.get("codex_token_url") or "https://auth.openai.com/oauth/token"),
            client_id=str(model_config.get("codex_client_id") or "app_EMoamEEZ73f0CkXaXp7hrann"),
        )
    ...
    async def call(self, params: Dict[str, Any]) -> CodexResult:
        ...
        try:
            payload = await self._post_with_retries(body=body, headers=headers)
            return self._parse_sse_to_result(payload)
            ...
            payload = await self._post_with_retries(body=body, headers=retry_headers)
            return self._parse_sse_to_result(payload)
    ...
    def _build_headers(
        ...
        *,
        ...
        if not account_id:
            raise CodexAuthError("Missing chatgpt account id. Re-run pi login.")
    ...
    async def _post_with_retries(self, *, body: Dict[str, Any], headers: Dict[str, str]) -> str:
        base_delay_seconds = 1.0
        url = self._resolve_codex_url(self.config.base_url)
        ...
        async with httpx.AsyncClient(timeout=self.config.timeout_seconds, transport=self._transport) as client:
            for attempt in range(self.config.max_retries + 1):
                ...
                raise CodexHTTPError(response.status_code, text, parsed)
    ...
    def _resolve_reasoning(self, params: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        effort = reasoning_obj.get("effort") or params.get("reasoning_effort") or self._model_config.get("codex_reasoning_effort")
        summary = (
    ...
    def _parse_error_response(self, status_code: int, text: str) -> str:
        ...
        if isinstance(parsed, dict):
            ...
            if code and any(token in code for token in ("usage_limit_reached", "usage_not_included", "rate_limit_exceeded")):
                ...
                friendly = "Codex authentication failed. Run pi login again."
