...
if [ "${INSTALL_MODE}" = "remote" ]; then
  REMOTE_INSTALL_SCRIPT_URL="${REMOTE_INSTALL_SCRIPT_URL:-https://github.com/memgrafter/flatmachines/releases/latest/download/install.sh}"
  REMOTE_MANIFEST_URL="${REMOTE_MANIFEST_URL:-https://github.com/memgrafter/flatmachines/releases/latest/download/manifest.json}"

  bash -lc "set -euo pipefail; curl -fsSL '${REMOTE_INSTALL_SCRIPT_URL}' | bash -s -- --manifest-url '${REMOTE_MANIFEST_URL}' --yes --skip-auth-setup --dir '${INSTALL_DIR}'"
else
